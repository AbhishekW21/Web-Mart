...
class ProductView(View):
	def get(self, request):
		...
		gadget = Product.objects.filter(category='G')
...
def mobile(request, data=None):
	...
	if data==None :
		mobiles = Product.objects.filter(category='M')
	elif data == 'redmi' or data == 'samsung' or data == 'realme' or data == 'oppo' or data == 'oneplus' or data == 'apple' :	
...
def laptop(request, data=None):
	...
	elif data == 'asus' or data == 'hp' or data == 'lenovo' or data == 'apple':
...
def gadget(request, data=None):
	totalitem = 0
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
	if data==None :
			gadget = Product.objects.filter(category='G')
	elif data == 'headphone' or data == 'earphone' or data == 'speaker' :
			gadget = Product.objects.filter(category='G').filter(brand=data)
	elif data == 'below':
			gadget = Product.objects.filter(category='G').filter(discounted_price__lt=1500)
	elif data == 'above':
			gadget = Product.objects.filter(category='G').filter(discounted_price__gt=1500)
	return render(request, 'app/gadget.html', {'gadget':gadget, 'totalitem':totalitem})
...
def shoe(request, data=None):
	...
	if data==None :
			shoe = Product.objects.filter(category='S')
	elif data == 'hrx' or data == 'nike':
			shoe = Product.objects.filter(category='S').filter(brand=data)
	elif data == 'below':
			shoe = Product.objects.filter(category='S').filter(discounted_price__lt=2500)
	elif data == 'above':
			shoe = Product.objects.filter(category='S').filter(discounted_price__gt=2500)
	return render(request, 'app/shoe.html', {'shoe':shoe, 'totalitem':totalitem})
